...
class Lines(Points):
    ...
    def __init__(self, lines = None, color = None, name = None, state = 1, transparency = 0, colormap = "RdYlBu_r", linewidth = 0.05, render_as = "cylinders", starts = None, ends = None, render_ends = False, *args, **kwargs) -> None:
    ...
    def _create_CGO_list(self) -> str:
        ...
        if self.render_as == "lines":
            ...
            triangles = np.hstack([
                np.full(cgo_vertices.shape[0], "ALPHA")[:,None], transparency[:,None], np.full(cgo_vertices.shape[0], "CONE")[:,None], cgo_vertices, np.full(cgo_vertices.shape[0], self.linewidth)[:,None], np.full(cgo_vertices.shape[0], self.linewidth)[:,None], cgo_colors, np.full((cgo_vertices.shape[0],2), (1.0, 1.0)) \
            ]).flatten()
            cgo_list.extend(triangles)
        if self.render_ends:
    ...
    def as_dotted(self, max_length = 0.1):
        ...
        return Lines(vertices, new_colors, self.name, self.state, np.array(new_transparency), self.colormap, self.linewidth, self.render_as)
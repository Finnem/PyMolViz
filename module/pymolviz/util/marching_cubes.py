"""Marching cubes isosurface on a cubic lattice.

Each cube is polygonised from its 12 edges (Lorensen & Cline). Shared faces of
adjacent cubes use the same interpolated vertices, so the mesh welds closed.
"""
...
def _build_tri_table():
    """Return a (256, 15) int8 table of edge indices, -1 terminated."""
    # Canonical triangles for cubeindex bits in Bourke order, cases 0–14 plus
    # complements. Values from Bhaniramka / Bourke polygonise.
    base = {
        0: (),
        1: (0, 8, 3),
        2: (0, 1, 9),
    ...
    table = np.full((256, 15), -1, dtype=np.int8)
    for idx, edges in base.items():
        for i, e in enumerate(edges):
            table[idx, i] = e
    return table


_TRI_TABLE = _build_tri_table()
...
def _interp(p0, p1, v0, v1):
    ...
    if abs(denom) < 1e-18:
        return np.asarray(p0, dtype=float)
    t = min(max(float(v0 / denom), 0.0), 1.0)
    return np.asarray(p0, dtype=float) + t * (np.asarray(p1, dtype=float) - np.asarray(p0, dtype=float))


def cube_triangles(pts8, vals8):
    """Triangles for one cube. ``pts8``/``vals8`` use x + 2y + 4z corner order."""
    pts = [np.asarray(pts8[_TO_BOURKE[i]], dtype=float) for i in range(8)]
    vals = [float(vals8[_TO_BOURKE[i]]) for i in range(8)]
    cubeindex = 0
    for i, value in enumerate(vals):
        if value < 0.0:
            cubeindex |= 1 << i
    edges = _TRI_TABLE[cubeindex]
    if int(edges[0]) < 0:
        return []
    cache = {}
    ...
    def vert(e):
        ...
        if hit is None:
            a, b = _EDGES[int(e)]
            hit = _interp(pts[a], pts[b], vals[a], vals[b])
            cache[int(e)] = hit
        return hit

    inside = [pts[i] for i, value in enumerate(vals) if value < 0.0]
    if not inside:
        return []
    inside_c = np.mean(inside, axis=0)
    tris = []
    trip = []
    for e in edges:
        if int(e) < 0:
        ...
        if len(trip) == 3:
            ...
            n = np.cross(b - a, c - a)
            centroid = (a + b + c) / 3.0
            if float(np.dot(n, centroid - inside_c)) < 0.0:
                tris.append((a, c, b))
            else:
                tris.append((a, b, c))
            trip = []

...
class IsoVolume(Volume):
    def __init__(self, regular_data : RegularData, name = None, colormap = "RdYlBu_r", alphas = None, clims = None, selection = None, carve = None, margin = 0.05, state = 0):
        """ 
        ...
        """
        if clims is None:
            clims = list(pd.qcut(regular_data.values, 7, retbins=True)[1])
            self.clims = clims
        else:
            self.clims = clims
        ...
        super().__init__(regular_data, name, colormap, alphas, clims, selection, carve, state)
